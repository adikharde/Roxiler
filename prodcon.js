...
const getTransactions = async (req, res) => {
    try {
        ...
        const products = await pm.find(query)
            .skip((currentPage - 1) * itemsPerPage)
            .limit(itemsPerPage)

        const totalProducts = await pm.countDocuments(query)
...
const getSalesStatistics = async (req, res) => {
    try {
        ...
        const unSoldItems = await pm.find({
            sold: false,
            $expr: {
                $eq: [{ $month: "$dateOfSale" }, parseInt(month)]
            }
...
const getBarChartData = async (req, res) => {
    try {
        ...
        const barChartData = await pm.aggregate([
            {
                $addFields: {
...
const getPieChartData = async (req, res) => {
    try {
        ...
        const pieChartData = await pm.aggregate([
            {
                $addFields: {
                    soldMonth: { $month: "$dateOfSale" }
                }
